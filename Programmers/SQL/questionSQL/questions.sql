...
INSERT INTO TB_CUSTOMER VALUES ('2019069','김한길','M','19920315',NULL,'hkkim@ssoya.com',15320,  '20190217110704');
INSERT INTO TB_CUSTOMER VALUES ('2019095','남궁소망','M','19620728','003-6273-8539',NULL,890,'20190312124558');
...
-- 고객포인트 테이블에서 등록일시가 2018년 내에 있고, 
-- 포인트가 10,000에서 50,000 포인트 범위의 데이터를 검색한다.
SELECT*FROM tb_point;

SELECT
*
FROM tb_point
WHERE `REG_DTTM` LIKE '2018%'
...
-- 연습문제 8. 비교연산자
-- 고객 테이블에서 누적포인트가 20,000 이상인 1980년대 남성 고객의 
-- 고객코드, 고객명, 성별, 생년월일, 누적포인트를 검색한다.
SELECT*FROM tb_customer;
...
-- 연습문제 9. MONTH, AND, IN
-- 고객 테이블에서 남성이면서 생년월일 중 월이 5, 6, 7월인 고객의 
-- 고객코드, 고객명, 성별, 생년월일, 누적포인트를 검색한다.
-- 답지랑 다를 수 있음
SELECT*FROM tb_customer;
...
FROM tb_customer
WHERE `MW_FLG` = 'M'
  AND MONTH(`BIRTH_DAY`) IN ('05','06','07')
  ;
...
FROM tb_customer
WHERE `MW_FLG` = 'M'
  AND MONTH(STR_TO_DATE(`BIRTH_DAY`,'%Y%m%d')) IN (5,6,7)
;
...
-- 연습문제 10. LIKE
-- 고객 테이블에서 고객코드가 ‘2017’로 시작하면서 남성인 고객 
-- 또는 고객코드가 ‘2019’로 시작하면서 여성인 고객을 구하고, 
-- 그 중 누적포인트가 30000 이하인 데이터를 검색한다.
SELECT*FROM tb_customer;

SELECT
*
FROM tb_customer
WHERE `TOTAL_POINT` <= 30000
  AND ((`MW_FLG`='M' AND `CUSTOMER_CD` LIKE '2017%')
       OR (`MW_FLG`='W' AND `CUSTOMER_CD` LIKE '2019%')
       )
;
...
-- 연습문제 12. IN 연산자 
-- 고객포인트 테이블에서 고객코드가 ‘2017042’ 또는 ‘2018087’ 또는 '2019095' 이면서 
-- 포인트내용에 ‘구매’ 문자가 포함된 데이터를 검색한다.
SELECT*FROM tb_point;
...
-- 연습문제 15. GROUP BY
-- 판매 테이블에서 판매일이 ‘20190802’ 또는 ‘20190803’을 대상으로 판매일과 상품명으로 그룹화해 
-- 총판매수를 구하고 판매일과 상품명은 가나다 순으로 보인다.
SELECT
  `SALES_DT`
  , `PRODUCT_NM`
...
-- 고객 테이블에서는 고객코드, 고객명, 성별을 검색한 후 
-- 고객포인트 테이블에서는 일련번호, 포인트내용, 포인트를 검색한다.
SELECT
  C.`CUSTOMER_CD`
  ,C.`CUSTOMER_NM`
  , C.`MW_FLG`
...
JOIN tb_point AS P
  ON C.`CUSTOMER_CD` = P.`CUSTOMER_CD`
WHERE C.`CUSTOMER_CD` = '2019069'
;



-- 연습문제 18. CASE
-- 고객 테이블에서 누적포인트가 1,000에서 20,000미만이면 “실버”, 
-- 20,000에서 50,000미만 이면 “골드”, 
-- 50,000이상이면 “VIP” 등급을 보이고 
...
SELECT
  ...
  , `TOTAL_POINT`
  , CASE
      WHEN `TOTAL_POINT` BETWEEN 1000 AND 19999 THEN '실버'
...
-- 연습문제 19. ROWNUM
-- 성적 테이블에서 반코드가 ‘A’ 또는 ‘C’반의 학생을 대상으로 
-- 모든 필드와 순차적인 행번호를 검색한다.
-- PDF에서 TOT AVG는 무시한다. 
SELECT
  ROW_NUMBER() OVER(ORDER BY `CLASS_CD`) AS '행 번호'
  , `CLASS_CD`
  , `STUDENT_NM`
  , `STUDENT_NO`
  , `KOR`
...
-- 고객 테이블에서 고객코드가 ‘2018’ 또는 ‘2019’로 시작하고, 
-- 생일이 1990년 또는 2000년 대인 고객 중 
-- 전화번호가 설정되어 있는 데이터를 검색한다.
SELECT
  `CUSTOMER_CD`
  , `CUSTOMER_NM`
  , `BIRTH_DAY`
...
WHERE (`CUSTOMER_CD` LIKE '2018%' OR `CUSTOMER_CD` LIKE '2019%')
  AND YEAR(STR_TO_DATE(`BIRTH_DAY`,'%Y%m%d')) BETWEEN 1990 AND 2009
